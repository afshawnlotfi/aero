...
Sidebar.Geometry.PropGeometry = function (editor, object) {
  ...
  var rotationType = new UI.Select()
    .setOptions({
      CCW: "CCW",
      CW: "CW",
    })
  ...
  var nblades = new UI.Integer(parameters.nblades)
    .setRange(1, 10)
    .onChange(update)
  ...
  var has_eMotor = new UI.Checkbox(
    parameters.electric_motor.has_motor
  ).onChange(update) // DFH
  ...
  var mr = new UI.Number(parameters.electric_motor.motor_resistance).onChange(
    update
  )
  ...
  var nlc = new UI.Number(
    parameters.electric_motor.motor_no_load_current
  ).onChange(update)
  ...
  var gr = new UI.Number(parameters.electric_motor.gear_reduction).onChange(
    update
  )
  ...
  var bv = new UI.Number(parameters.electric_motor.battery_voltage).onChange(
    update
  )
  ...
  var br = new UI.Number(parameters.electric_motor.battery_resistance).onChange(
    update
  )
  ...
  var scr = new UI.Number(
    parameters.electric_motor.speed_control_resistance
  ).onChange(update)
  scrRow.add(
    new UI.Text("Speed Controller Resistance (ohms)").setWidth("300px")
  )
  scrRow.add(scr)
  eMotor.add(scrRow)
