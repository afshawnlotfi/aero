...
*/
  ...
  function update() {
    ...
    salphaRow.setDisplay(
      type === "stallonset" || type === "stallairspeed" ? "" : "none"
    )
    sLRow.setDisplay(
      type === "stallonset" || type === "stallairspeed" ? "" : "none"
    )
    ...
    LtrimRow.setDisplay(
      type === "pitchtrim" || type === "targetl" ? "" : "none"
    )
    mtrimRow.setDisplay(type === "pitchtrim" ? "" : "none")
    talphaRow.setDisplay(
      type === "pitchtrim" || type === "targetl" ? "" : "none"
    )
  ...
  function run_callback(result) {
    ...
    if (result.error == undefined) {
      ...
      switch (type) {
        case "forces":
          ...
          if ("total" in result.wing_results) {
            ...
            WLD.setValue(
              (
                result.wing_results.total.FL / result.wing_results.total.FD
              ).toFixed(4)
            )
          } else {
        ...
        case "stallonset":
          ...
          if (result.alpha == -1.0)
            alert(
              "Error: No airfoil CLmax specified. Check your airfoil settings."
            )
          break
        case "stallairspeed":
      ...
      switch (result.error) {
        case "unliftable":
          msg =
            "Aw Snap. Your aircraft is having trouble producing enough lift. Try adding more wing area or increasing the velocity."
          confirm(msg)
          break
        case "untrimmable":
          msg =
            "Aw Snap. Your aircraft is having trouble being trimmed. Try increasing elevator size or changing target values."
          confirm(msg)
          break
        case "unstallable":
        ...
        case "underivable":
          msg =
            "Aw Snap. Aircraft must have wings to run derivatives analysis. \nAdd wings or run different analysis"
          confirm(msg)
          break
        case "no_elevator":
          msg =
            "Aw Snap. Your aircraft cannot be trimmed because it has no elevator. Please add an elevator."
          confirm(msg)
          break
        case "upgrade":
          msg =
            'Aw Snap. This feature requires an upgrade to MachUp Plus.\n\nClick "OK" to upgrade to MachUp Plus.'
          if (confirm(msg)) {
