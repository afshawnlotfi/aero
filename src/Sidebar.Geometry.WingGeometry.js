...
Sidebar.Geometry.WingGeometry = function ( editor, object ) {
	...
	var is_main_wing = new UI.Checkbox( parameters.is_main ).setMarginLeft( '-40px' ).onChange( update );
	...
	var sideType = new UI.Select().setOptions( {
	...
	var sweep = new UI.Number( parameters.sweep ).setRange( -80.0, 80.0 ).onChange( update );
  ...
  // --------------------------------------
	...
	var nAFseg = new UI.Select().setOptions( {"50" : "50", "100" : "100", "200" : "200"} ).setWidth( '80px' ).setColor( '#444' ).setFontSize( '12px' )
  ...
  controlSurface.setCollapsed(true);
	...
	var has_control_surface = new UI.Checkbox(parameters.control.has_control_surface).onChange( updateCS );
  ...
  cs_spantipRow.setDisplay( 'none' );
	...
	var cs_chordroot = new UI.Number( parameters.control.chord_root ).onChange( update );
  ...
  cs_chordtipRow.setDisplay( 'none' );
	...
	var mixElevator = new UI.Number( parameters.control.mix.elevator ).onChange( update );
  ...
  rudderRow.setDisplay( 'none' );
	...
	var mixAileron = new UI.Number( parameters.control.mix.aileron ).onChange( update );
	...
	function update() {
        ...
        if(object.parent.type == "Mesh"){
            if(object.parent.geometry.type == "WingGeometry") {
				...
				if ( sameAsRoot.dom.checked === true ) { // DFH
