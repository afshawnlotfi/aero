//--------------------------------------------------
THREE.WingGeometry=function(is_main,side,span,sweep,dihedral,mount,washout,root_chord,tip_chord,root_airfoil,tip_airfoil,nSeg,nAFseg,left_start,right_start,dy,control,same_as_root){ // DFH
    ...
    if(side == "both")  {startwing = 1; stopwing = 2;};
    if(side == "right") {startwing = 1; stopwing = 1;};
    if(side == "left")  {startwing = 2; stopwing = 2;};
    ...
    for(iwing=startwing;iwing<=stopwing;iwing++){
        ...
        csrooti = csrooti - 1;
        if(mindisttip  > 0) {cstipi  = cstipi  - 1;}
        ...
        for (iloop = 0; iloop < nloop; iloop++) {
            ...
            if(control.has_control_surface) {
                if(iloop == 0) {loopstart = 0;          loopend = csrooti;} // paint inboard of flap
                if(iloop == 1) {loopstart = csrooti+1;  loopend = cstipi;} // paint flap
                if(iloop == 2) {loopstart = cstipi+1;   loopend = nSeg;} // paint outboard of flap
            }
    ...
    //            math_rot_z(fvec,sweep*Math.PI/180.0); // Fix this? Probably should also rotate by sweep at control surface hinge line
                ...
                if(my_side == "left") {qvec.y = -qvec.y; fvec.y = -fvec.y;}
                ...
                for ( x = 0; x <= nairfoilpts-1; x ++ ) {
                    ...
                    var airfoilx = my_root_airfoil[1][x] + percent*(my_tip_airfoil[1][x] - my_root_airfoil[1][x]); // interpolates
                    var airfoily = my_root_airfoil[2][x] + percent*(my_tip_airfoil[2][x] - my_root_airfoil[2][x]); // interpolates
    ...
    */
                    ...
                    if(my_side == 'left'){
                        if(y == loopend-1)    {n2 = n1.clone(); n3 = n4.clone();}
                        if(y == loopstart)    {n1 = n2.clone(); n4 = n3.clone();}
                    } else {
                        if(y == loopstart)    {
    ...
    //                        console.log('next');
                        ...
                        if(y == loopend-1) {n1 = n2.clone(); n4 = n3.clone();}
                    }
...
//console.log(childObject.type, parentObject.type)
        if(childObject.type == 'Mesh'){
            if((childObject.geometry.type == 'WingGeometry') || (childObject.geometry.type == 'PropGeometry')) {
                ...
                if(parentObject.type == 'Light') {ok_2_connect = 0;}
//different from mu1
                if(parentObject.type == 'Group') {ok_2_connect = 0;}
            }
        }
    } else {ok_2_connect = -1};
...
// Propeller Geometry
//-------------------------------
THREE.PropGeometry=function(nblades,rotation,diameter,pitch,hub_radius,root_chord,tip_chord,elliptical,root_airfoil,tip_airfoil,electric_motor,nSeg,nAFseg,same_as_root){
    THREE.Geometry.call(this);
...
//            var v = y/nSeg;
        ...
        for ( x = 0; x < nairfoilpts-1; x ++ ) {
            ...
            for ( y = 0; y < nSeg; y ++ ) {
                ...
                if(my_side == 'left'){
                    if(y == nSeg-1) {n2 = n1.clone(); n3 = n4.clone();}
                    if(y == 0)    {n1 = n2.clone(); n4 = n3.clone();}
                } else {
                    if(y == 0)    { n2 = n1.clone(); n3 = n4.clone();                    }
                    if(y == nSeg-1) {n1 = n2.clone(); n4 = n3.clone();}
                }
...
//          async: false,
          ...
          error: function(xhr, status, error) { console.log('Error in ajax call to server.'); alert('Error: '+ xhr.status+ ' - '+ error); },
...
console.log(parameters);
	    ...
	    for (var tip_name in parameters.tip_airfoil) break;
            ...
            global_wings[childObject.name] = { name:childObject.name,
                            ID:global_wing_ID,
                            is_main:my_is_main,
...
//    console.log('looking at object:',myObject.name)
    ...
    for (var ichild = 0; ichild < myObject.children.length; ichild++) {
        ...
        if((childObject.type == "Mesh") && (childObject.geometry.type == "PropGeometry")) {
            ...
            global_props[childObject.name] = { name:childObject.name,
...
var calculate_reference_values = function ( editor ) {
    ...
    if(isNaN(analysis.reference.longitudinal_length)) {analysis.reference.longitudinal_length = 0.0;}
