...
var unitType = new UI.Select().setOptions( {
	'English':  'English',
	'SI': 'SI'
...
var velocity = new UI.Number( analysis.condition.velocity ).onChange( updateCondition );

var densityText = new UI.Text( 'Density (slugs/ft^3)' ).setWidth( '140px' );
var density = new UI.Number( analysis.condition.density ).onChange( updateCondition );
...
var elevator = new UI.Number( analysis.controls.elevator.deflection ).onChange( updateCondition );
var rudder = new UI.Number( analysis.controls.rudder.deflection ).onChange( updateCondition );
var aileron = new UI.Number( analysis.controls.aileron.deflection ).onChange( updateCondition );
var flap = new UI.Number( analysis.controls.flap.deflection ).onChange( updateCondition );

var propRot = new UI.Integer( analysis.prop_controls.rot_per_sec *60 ).setRange(0,Infinity).onChange( updateCondition );
var propRotRow = new UI.Row().setMarginLeft( '10px' );
var throttleRow = new UI.Row().setMarginLeft( '10px' );
var throttle = new UI.Integer( analysis.prop_controls.electric_throttle *100).setRange(0,100).onChange( updateCondition );
var indivControl = new UI.Checkbox(analysis.prop_controls.individual_control).onChange( updateCondition );
...
function updateCondition() {
    ...
    if((density.getValue() == 0.0023769)||(density.getValue() == 1.225)){
        if (unitType.getValue() == 'English'){analysis.condition.density = 0.0023769}
        else{analysis.condition.density = 1.225}
