...
Menubar.Add = function ( editor ) {
	...
	option.onClick( function () {
				...
				if(tempObject !== null){
						if(tempObject.geometry !== undefined){
								...
								console.log('parent is a wing');
										...
										mount = tempObject.geometry.parameters.mount - tempObject.geometry.parameters.washout;
		...
		var geometry = new THREE.WingGeometry( is_main,side,span,sweep,dihedral,mount,washout,root_chord,tip_chord,root_airfoil,tip_airfoil,nSeg,nAFseg,left_start,right_start,dy,control,same_as_root );
	...
	option.onClick( function () {
		...
		var geometry = new THREE.PropGeometry( nblades,rotation,diameter,pitch,hub_radius,root_chord,tip_chord,elliptical,root_airfoil,tip_airfoil,electric_motor,nSeg,nAFseg,same_as_root);
	...
	option.onClick( function () {
		...
		editor.execute( new AddObjectCommand( mesh ) );
		editor.select( mesh ); // DFH
        if(tempObject !== null) editor.parent( mesh, editor.scene.getObjectById( tempObject.id, true ) ); // DFH
